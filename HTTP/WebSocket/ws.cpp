...
void appendRootRow(std::vector<FsListRow>& rows, std::set<std::string>& seen, const fs::path& path) {
...
std::vector<FsListRow> listRootRows() {
    ...
    while (mounts >> device >> mountPoint >> fsType) {
        ...
        if (
            ...
            appendRootRow(rows, seen, fs::path(mountPoint));
        }
    }
#elif defined(__APPLE__)
    appendRootRow(rows, seen, fs::path("/"));
    std::error_code volumesEc;
    for (fs::directory_iterator it(fs::path("/Volumes"), fs::directory_options::skip_permission_denied, volumesEc), end;
         it != end; it.increment(volumesEc)) {
        if (volumesEc) {
            volumesEc.clear();
            continue;
        }
...
void KeeplyAgentWsClient::handleServerMessage_(const std::string& payload){
    ...
    try{
        ...
        if(payload=="fs.disks"){sendJson_(buildFsDisksJson_(""));return;}
        ...
        if(payload.rfind("fs.disks:",0)==0){
            const std::string requestId=payload.substr(std::string("fs.disks:").size());
            sendJson_(buildFsDisksJson_(requestId));
            return;
        }
