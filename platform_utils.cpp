...
fs::path normalizedOrEmpty(const fs::path& path) {
...
void appendUnique(std::vector<fs::path>& out, const fs::path& path) {
    if (path.empty()) return;
...
#ifdef _WIN32
std::optional<fs::path> knownFolderPath(REFKNOWNFOLDERID folderId) {
    PWSTR raw = nullptr;
...
#if !defined(_WIN32)
std::optional<std::string> readXdgUserDir(const fs::path& homeDir, const std::string& key) {
...
std::optional<fs::path> homeFromEnvironment() {
#ifdef _WIN32
...
std::optional<fs::path> knownDirectoryPath(KnownDirectory dir) {
    ...
    switch (dir) {
        ...
        case KnownDirectory::Temp:
            if (const auto temp = envValue("TEMP")) return normalizedOrEmpty(pathFromUtf8(*temp));
            if (const auto tmp = envValue("TMP")) return normalizedOrEmpty(pathFromUtf8(*tmp));
            return std::nullopt;
    }
    return std::nullopt;
#else
    ...
    switch (dir) {
        case KnownDirectory::Home:
            if (const auto value = homeFromEnvironment()) return *value;
            return fallbackCurrentPath();
        case KnownDirectory::Desktop:
            if (const auto value = readXdgUserDir(home, "XDG_DESKTOP_DIR")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / "Desktop");
        case KnownDirectory::Documents:
            if (const auto value = readXdgUserDir(home, "XDG_DOCUMENTS_DIR")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / "Documents");
        case KnownDirectory::Downloads:
            if (const auto value = readXdgUserDir(home, "XDG_DOWNLOAD_DIR")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / "Downloads");
        case KnownDirectory::Pictures:
            if (const auto value = readXdgUserDir(home, "XDG_PICTURES_DIR")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / "Pictures");
        case KnownDirectory::Music:
            if (const auto value = readXdgUserDir(home, "XDG_MUSIC_DIR")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / "Music");
        case KnownDirectory::Videos:
            if (const auto value = readXdgUserDir(home, "XDG_VIDEOS_DIR")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / "Videos");
        case KnownDirectory::LocalData:
#if defined(__APPLE__)
            return normalizedOrEmpty(home / "Library" / "Application Support");
#else
            if (const auto value = envValue("XDG_DATA_HOME")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / ".local" / "share");
#endif
        case KnownDirectory::StateData:
#if defined(__APPLE__)
            return normalizedOrEmpty(home / "Library" / "Application Support");
#else
            if (const auto value = envValue("XDG_STATE_HOME")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / ".local" / "state");
#endif
        case KnownDirectory::CacheData:
#if defined(__APPLE__)
            return normalizedOrEmpty(home / "Library" / "Caches");
#else
            if (const auto value = envValue("XDG_CACHE_HOME")) return normalizedOrEmpty(pathFromUtf8(*value));
            return normalizedOrEmpty(home / ".cache");
#endif
        case KnownDirectory::Temp:
            if (const auto value = envValue("TMPDIR")) return normalizedOrEmpty(pathFromUtf8(*value));
            return std::nullopt;
    }
    return std::nullopt;
#endif
...
#endif
    if (const auto home = homeFromEnvironment()) return *home;
    return fallbackCurrentPath();
}
...
fs::path defaultKeeplyDataDir() {
    ...
    return fallbackCurrentPath() / ".keeply";
}
...
#ifdef _WIN32
    if (const auto systemRoot = envValue("SystemRoot")) appendUnique(out, pathFromUtf8(*systemRoot));
    if (const auto windir = envValue("WINDIR")) appendUnique(out, pathFromUtf8(*windir));
    if (const auto programFiles = envValue("ProgramFiles")) appendUnique(out, pathFromUtf8(*programFiles));
    if (const auto programFilesX86 = envValue("ProgramFiles(x86)")) appendUnique(out, pathFromUtf8(*programFilesX86));
    if (const auto programData = envValue("ProgramData")) appendUnique(out, pathFromUtf8(*programData));
    appendUnique(out, defaultKeeplyTempDir());
    if (const auto home = homeDirectoryPath(); !home.root_path().empty()) {
        appendUnique(out, home.root_path() / "$Recycle.Bin");
        appendUnique(out, home.root_path() / "System Volume Information");
    }
#else
    appendUnique(out, "/proc");
    appendUnique(out, "/sys");
    appendUnique(out, "/dev");
    appendUnique(out, "/run");
    appendUnique(out, "/tmp");
    appendUnique(out, "/mnt");
    appendUnique(out, "/media");
    appendUnique(out, "/lost+found");
    appendUnique(out, "/var/run");
    appendUnique(out, "/var/tmp");
#endif
    return out;
}
...
#ifdef _WIN32
    const std::wstring wideMode = widenAscii(mode);
